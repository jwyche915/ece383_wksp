	...
	while(1) {
    	...
    	switch(c) {
    		...
    		case '?':
    			...
    			printf("d: REGISTER values from OScope\r\n");
    		...
    		/*-------------------------------------------------
    		 * Debug...view register values read into Microblaze
    		 * -----------------------------------------------*/
    		case 'd':
    			printf("current register values read into Microblaze from OScope\r\n");
    			printf("Voltage Trigger (slv_reg11): %d\r\n",Xil_In32(trig_volt_Reg));
    			printf("Time Trigger (slv_reg12): %d\r\n",Xil_In32(trig_time_Reg));
    			printf("Audio Codec Left Output (LBus_out[15:0] : slv_reg5): %d\r\n",Xil_In32(Lbus_out_Reg));
    			printf("Audio Codec Left Output (LBus_out[15:7] : slv_reg5): %d\r\n",(Xil_In32(Lbus_out_Reg)/128));
    			printf("Audio Codec Right Output (RBus_out[15:0] : slv_reg6): %d\r\n",Xil_In32(Rbus_out_Reg));
    			printf("Audio Codec Right Output (RBus_out[15:7] : slv_reg6): %d\r\n",(Xil_In32(Rbus_out_Reg)/128));
    			break;
