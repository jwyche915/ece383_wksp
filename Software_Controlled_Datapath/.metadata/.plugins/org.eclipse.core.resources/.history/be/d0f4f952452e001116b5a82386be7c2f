 ...
 */
#define countBase		0x44a0002C//0x44a00000
#define countQReg		0x44a0002C//countBase			// 8 LSBs of slv_reg0 read=Q, write=D
#define	countCtrlReg	0x44a0002C//countBase+4			// 2 LSBs of slv_reg1 are control
...
int main(void) {
    ...
    while(1) {
		...
		switch(c) {
			 ...
			 * clear flag, fill buffer, and print buffer to UART
			 * -------------------------------------------------------*/
			case 'm':
			...
			/*--------------------------------------------------------
			 * WRITE TO BRAM...write samples from option 'm' into
			 * datapath BRAM
			 * ------------------------------------------------------*/
			case 'w':
				printf("\r\nWriting to BRAM\r\n");
			...
			/*-----------------------------------------------------
			 * case ‘z’: Given this trigger location in the Array_L,
			 * write the appropriate data values in the Array_L to
			 * the BRAM, so the sine wave appears triggered.
			 * ---------------------------------------------------*/
			case 'z':
				trigger_voltage = Xil_In16(TRIG_VOLT_REG);
				trigger_time = Xil_In16(TRIG_TIME_REG);
